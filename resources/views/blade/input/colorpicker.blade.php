...
<div {{ $attributes->merge(['class' => 'color input-group colorpicker-component row col-md-5']) }} id="{{ $div_id }}">
    <input class="form-control" placeholder="{{ $placeholder }}" aria-label="{{ $name }}" name="{{ $name }}" type="text" id="{{ $id }}" value="{{ old($name, $item->{$name} ?? $default ?? '') }}" {{ (config('app.lock_passwords')===true) ? ' disabled' : '' }}>
